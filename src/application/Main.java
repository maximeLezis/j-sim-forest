...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			/*
			 * BorderPane root =
			 * (BorderPane)FXMLLoader.load(getClass().getResource("../view/Sample.fxml"));
			 * Scene scene = new Scene(root,1200,600);
			 * scene.getStylesheets().add(getClass().getResource("../view/application.css").
			...
			Scene scene = new Scene(root, 1200, 600);
			primaryStage.setTitle("J-Sim Forest");
 ...
 * //scene.getStylesheets().add(getClass().getResource("../view/application.css"
 * ).toExternalForm()); //primaryStage.setScene(scene); //primaryStage.show();
 * GridPane grid = new GridPane(); Scanner sc = new Scanner(System.in);
 * System.out.
 * println("Saisissez la taille du plateau (entrez une valeur puisque cela sera un carré) : "
 * ); int i = sc.nextInt(); int j = sc.nextInt();
 * 
 * for (int row = 0; row < (i + 1); row++) { for (int col = 0; col < (j + 1);
 * col++) { if (row != i) { Rectangle rec = new Rectangle(); rec.setWidth(10);
 * rec.setHeight(10); if ((row % 2 == 0 && col % 2 == 0) || (row % 2 != 0 && col
 * % 2 != 0)) { rec.setFill(Color.RED); } else { rec.setFill(Color.GREEN); }
 * GridPane.setRowIndex(rec, row); GridPane.setColumnIndex(rec, col);
 * grid.getChildren().addAll(rec); } } }
 * 
 * Scene scene = new Scene(grid, (i+1)*10, (i+1)*10);
 * 
 * primaryStage.setTitle("Jeu de foret"); primaryStage.setScene(scene);
 * 
